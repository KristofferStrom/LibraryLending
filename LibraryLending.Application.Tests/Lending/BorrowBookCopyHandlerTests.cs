...
{
    [Fact]
    public async Task Handle_WhenMemberNotFound_ShouldReturnMemberNotFound_AndNotSave()
    {
        // Arrange
        ...
        var command = CommandFactory.BorrowBookCopy(memberNumber: "M-404", copyBarcode: "BC-0001");
    ...
    [Fact]
    public async Task Handle_HappyPath_ShouldAddLoan_UpdateCopyStatus_AndSave()
    {
        // Arrange
        var fx = new ApplicationTestFixture();
        var handler = new BorrowBookCopyHandler(
        ...
        var command = CommandFactory.BorrowBookCopy("M-0001", "BC-0001");

        var memberNumber = MemberNumber.Create(command.MemberNumber).Value;
        var barcode = CopyBarcode.Create(command.CopyBarcode).Value;

        var member = DomainFactory.ActiveMember("M-0001");
        var copy = DomainFactory.AvailableCopy(barcode: "BC-0001");

        // Om LendingPolicy.Rehydrate är internal: Domain måste ha InternalsVisibleTo("LibraryLending.Testing")
        var policy = DomainFactory.ActivePolicy(loanDays: 14);

        fx.Members.GetByMemberNumberAsync(memberNumber, Arg.Any<CancellationToken>())
                  .Returns(member);

        fx.BookCopies.GetByBarcodeAsync(barcode, Arg.Any<CancellationToken>())
                     .Returns(copy);

        fx.LendingPolicies.GetCurrentAsync(Arg.Any<CancellationToken>())
                          .Returns(policy);
        ...
        fx.Loans.Received(1).Add(Arg.Is<Loan>(l =>
            l.MemberId == member.Id &&
            l.BookCopyId == copy.Id &&
            l.IsActive));
